...
export const ProductQuickView: React.FC<ProductQuickViewProps> = ({
  ...
  const getInitialImage = () => {
    if (product.availableFabrics[0] && product.swatchImageMap?.[product.availableFabrics[0].id]) {
      return product.swatchImageMap[product.availableFabrics[0].id];
    }
    if (product.availableWoodFinishes[0] && product.swatchImageMap?.[product.availableWoodFinishes[0].id]) {
      return product.swatchImageMap[product.availableWoodFinishes[0].id];
    }
  ...
  const handleWoodSelect = (wood: Swatch) => {
    ...
    if (product.swatchImageMap?.[wood.id]) {
      setSelectedImage(product.swatchImageMap[wood.id]);
    }
  };

  const handleFabricSelect = (fabric: Swatch) => {
    setSelectedFabric(fabric);
    if (product.swatchImageMap?.[fabric.id]) {
      setSelectedImage(product.swatchImageMap[fabric.id]);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-[#E6DDD0] overflow-hidden my-8 max-h-[90vh] flex flex-col md:flex-row">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 bg-white/80 hover:bg-[#2B2220] hover:text-white rounded-full transition-colors text-[#2B2220] shadow-sm cursor-pointer"
        >
          <X size={20} />
        </button>

        {/* Left: Image Gallery */}
        <div className="w-full md:w-1/2 bg-[#FAF6F0] p-6 flex flex-col justify-between">
            ...
            <button
              ...
              className={`absolute top-3 left-3 p-2 rounded-full shadow-md transition-all ${
                isWishlisted
                  ...
                  : 'bg-white text-[#2B2220] hover:text-[#C17D3C]'
              }`}
            >
          ...
          <div className="mt-4 p-4 bg-white rounded-xl border border-[#E6DDD0] text-xs text-[#2B2220] space-y-2.5 shadow-xs">
            ...
            <div className="pt-1.5 border-t border-[#FAF6F0] flex items-center justify-between text-[11px] text-[#6B5B54]">
              ...
              {product.dimensions.seatingCapacity && (
                <span><strong>Seating Capacity:</strong> {product.dimensions.seatingCapacity} Persons</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-8 overflow-y-auto space-y-6">
          <div>
            ...
            <h2 className="font-serif-brand text-2xl sm:text-3xl font-medium text-[#2B2220] mt-1">
              {product.name}
            </h2>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg border text-left flex items-center gap-2 transition-all cursor-pointer ${
                      selectedWood?.id === wood.id
                        ? 'border-[#C17D3C] bg-[#C17D3C]/5 font-semibold'
